...
# En este codigo se evalua unicamente el avance de la formacion sin la implementacion
...
def computegradient(x1,x2,robots,positions,desv,c,rota,p):
    ...
    for k in range(N):
        ...
        f_c = f1
        gradest[k,0] = f_c*(positions[k,0]-x1)
        gradest[k,1] = f_c*(positions[k,1]-x2)
    grad_est = constante*sum(gradest)
    return grad_est
