...
while(runsim):
    ...
    if (la.norm(error_theta) < 0.1 and fun < 0.999):
        # Calculo gradiente
        gradestfin=grad.computegradient(xo,yo,positions_agents,ro,center,num_of_agents,desv,rota,p) # Gradiente (paso el centro para que dentro calcule el valor de la función con el máximo en otro lado, en lugar de en (0,0))
        # Caso inicial (una única fuente).
        fun = grad.function(xo-CENTERX,yo-CENTERY,0,desv,p,0)
        # Parte unica fuente
        # fun = grad.function(xo-CENTERX,yo-CENTERY,rota,desv,p,0) # Gradiente.
        grd = grad.function(xo-CENTERX,yo-CENTERY,rota,desv,p,1)
        # Parte Multi-fuente.
        # f1 = grad.function(xo-CENTERX,yo-CENTERY,rota-np.pi/6,desv,p,0) # Gradiente.
        # f2 = grad.function(xo,yo-1200,rota,[300/np.sqrt(2),300/np.sqrt(2)],0.9*p,0) 
        # f3 = grad.function(xo-1200,yo,rota,[300/np.sqrt(2),300/np.sqrt(2)],0.9*p,0)
        # fun = f1+f2+f3
        # Gradiente multifuente
        # grd1 = grad.function(xo-CENTERX,yo-CENTERY,rota-np.pi/6,desv,p,1)
        # grd2 = grad.function(xo,yo-1200,rota,[300/np.sqrt(2),300/np.sqrt(2)],0.9*p,1)
        # grd3 = grad.function(xo-1200,yo,rota,[300/np.sqrt(2),300/np.sqrt(2)],0.9*p,1)
        # grd = grd1+grd2+grd3
        # Avance.
        ...
        if counter%20==0:     
          ...
          # # pl.arrow(ck[0],ck[1],k*gradestfin[0],1000*gradestfin[1],color='b')
          ...
          # pl.plot(counter,k*gradestfin[0],'.r',markersize=2)
          # pl.plot(counter,k*gradestfin[1],'.b',markersize=2)
