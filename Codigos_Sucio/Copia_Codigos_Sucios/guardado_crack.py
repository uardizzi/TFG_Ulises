...
desv = np.array([800/np.sqrt(2),400/np.sqrt(2)])
c = np.array([600,600])
...
def computegradient(x1,x2,robots,positions,desv,c,rota,p):
    ...
    for k in range(N):
        fradios = function(positions[k,0]-c[0],positions[k,1]-c[1],rota,desv,p,0) 
...
while(fun < 0.99999):
    ...
    fun = function(ck[0]-c[0],ck[1]-c[1],rota,desv,p,0)
    print("soy fun: ",fun)
